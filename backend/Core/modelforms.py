...
from .models import ImpactChallengeTable, MUNChallengeTable
...
class BaseChallengeModelAdminForm(forms.ModelForm):
    def clean(self):
        ...
        combination = self.Meta.model.objects.filter(
            committee=c, portfolio=p, status="AL"
        )
        if combination:
            raise forms.ValidationError(
                "This combination of committee and portfolio is already alloted"
            )
